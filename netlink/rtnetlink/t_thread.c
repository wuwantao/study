...
void *scan_netlink_info(void *args)
{
    int                socket_fd;
    int                err = 0;
    fd_set             rd_set;
    ...
    if (setsockopt(socket_fd, SOL_SOCKET, SO_RCVBUF, &len, sizeof(len)) < 0) {
    ...
    sa.nl_pid    = pthread_self() << 16 | getpid();
...
int main(int argc, char *argv[])
{
    ...
    ret = pthread_create(&tid, NULL, scan_netlink_info, NULL);
