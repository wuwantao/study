...
int main(int argc, char **argv)
{
    ...
    if (result) {
        ...
        xmlNodePtr child_pos;//声明一个临时的当前结点指针
        ...
        for (i = 0; i < nodeset->nodeNr; i++) {
            ...
            while (cur != NULL) {
                ...
                child_pos = cur->xmlChildrenNode;

                while (child_pos != NULL) {
                    if (!xmlStrcmp(child_pos->name, (const xmlChar *)"id"))
                        printf("id:%s\n", ((char*)XML_GET_CONTENT(child_pos->xmlChildrenNode)));
                    else if (!xmlStrcmp(child_pos->name, (const xmlChar *)"price"))
                        printf("price:%s\n", ((char*)XML_GET_CONTENT(child_pos->xmlChildrenNode)));
