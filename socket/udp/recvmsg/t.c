...
int main ( int argc, char **argv )
{
    ...
    peeraddr.sin_port=htons ( atoi ( "9999" ) );
    ...
    localaddr.sin_port=htons ( atoi ( "8888" ) );
    ...
    //发一个消息给目标主机
    while(0) {
        if ( sendto ( sockfd, "HELLO", strlen ( "HELLO" ), 0, (struct sockaddr*)&peeraddr, socklen )
                <0 )
        {
            perror("error:");
            //printf("error:%s\n", strerror(errno));
    ...
    for ( ;; )//接收消息循环
    {
        ...
        if ( sendto ( sockfd, "Hello", strlen ( buffer ), 0, (struct sockaddr*)&peeraddr, socklen)
                <0 )
        {
            printf ( "sendto err in udptalk!\n" );
            exit ( 3 );
